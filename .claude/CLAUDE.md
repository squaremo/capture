...
- Item states: `pending` → `triaged` / `reminder` / `urgent` / `awaiting_approval` / `checklist` → `acted` / `vetoed` / `failed` (a `checklist` item is terminal in its own right — it doesn't resolve on to `acted`/`vetoed`/`failed`)
