...
- **Checklists**: a checklist is just an item whose `text` *is* a markdown task list (`- [ ] goggles`), rendered as real checkboxes (`parseChecklist()`/`serializeChecklist()`/`renderChecklist()` in `item.js`) instead of the usual result strip. Sits in its own always-visible "checklists" inbox group (`item.status === 'checklist'`), separate from needs-attention/resolved, since it never resolves away. Ticking a box or hitting "reset" flips the relevant line(s) and PATCHes the whole `text` back — no dedicated checklist endpoint, no separate template/run distinction: resetting *is* how you reuse it next time. Saying a checklist's name with no items (e.g. "swimming checklist") does the same reset, but via a capture: `find_checklist` (read-only) looks up the existing item by title, `recall_checklist` (terminal) resets it in place — the capture itself just logs an `acted` audit entry, it doesn't spawn a second checklist.
...
- **`save_checklist` → `checklist`**: a terminal tool (no external side effect, like `save_to_inbox`) that rewrites the item's own `text` into a markdown task list instead of leaving the raw capture text in place — the `extra()` hook a terminal tool can define in `TOOL_REGISTRY` (`claude.js`), merged into `runProgram()`'s resolved output (now awaited, since a hook can have to look up/mutate another item — see below). `PATCH /api/items/:id` accepts `text` (alongside `status`/`tags`/`action_result`) so the frontend can tick/reset a checklist by writing the whole task list back.
- **`find_checklist` → `recall_checklist`**: recalling a checklist by name (e.g. "swimming checklist", no items listed) resets the *existing* item rather than creating another one — the same "avoid a duplicate" shape `search_linear_issues`/`create_linear_task` already use. `find_checklist` (read-only) matches the query against existing `checklist` items' titles; `recall_checklist` (terminal, no approval gate — resetting your own checklist isn't an external side effect) resets that item's text to all-unchecked via `db.js`, and the *new* capture item just resolves to `acted` with an audit `action_result` ("Reset \"Swimming kit\" checklist"). `unless found`, the plan falls back to `save_to_inbox` noting nothing matched.
