...
- **Preliminary work before deciding the action** — some captures might need investigation before the right action is even known (e.g. looking something up, disambiguating what's meant). Might need either a series of tool calls with wiring between them, or a preliminary research/reasoning step before the single action response — as opposed to today's one-shot "pick a tool now" call.
